...
<head>
    <?php
        if (strpos($_SERVER['HTTP_HOST'], 'www') === false && ( $_SERVER['HTTP_HOST'] != "localhost" && $_SERVER['HTTP_HOST'] != "rehab.mycoolmac.net" && $_SERVER['HTTP_HOST'] != "127.0.0.1") ){
    ...
    <script src="<?php echo base_url('application/js/fullcalendar.min-3.9.0.js');?>"></script>
    <script src="<?php echo base_url('application/js/bootstrap-editable.js');?>"></script>
    <script src="<?php echo base_url('application/js/colorpicker.js');?>"></script>
    <script src="<?php echo base_url('application/js/fine-uploader.min.js');?>"></script>
    <script src="<?php echo base_url('application/js/modernizr.custom-orig.js');?>"></script>
            ...
            window.fbAsyncInit = function() {
                ...
                FB.api('/' + facebookPageId + '',{fields:'posts.limit(4){message,actions,full_picture,picture,attachments{subattachments},shares,created_time,from,permalink_url,name,likes.limit(0).summary(true)}',access_token:''+ facebookAuthToken +''} , function(response){
                    ...
                    for ( var i = 0; i < response.posts.data.length; i++ ){
                        if ( typeof response.posts.data[i].shares === 'undefined') { shareCount = '0'; }
                        else { shareCount = response.posts.data[i].shares.count; }
                        fbHTML += '<li class="post">';
                        fbHTML += '     <div class="item">';
                        fbHTML += '         <div class="picture" style="background-image: url('+ response.posts.data[i].full_picture +');"><span class="notify-badge"><a class="fas fa-thumbs-up mr-1"></a>'+response.posts.data[i].likes.summary.total_count +'<i class="fas fa-share ml-2 mr-1"></i>'+ shareCount +'</span></div>';
                        fbHTML += '         <div class="container info text-justify" ><span class="notify-badge"><a href="' + response.posts.data[i].actions[0].link + '" target="_blank" class="fas fa-thumbs-up mr-1"></a>'+response.posts.data[i].likes.summary.total_count+'<a href="' + response.posts.data[i].actions[2].link + '" target="_blank" class="fas fa-share ml-2 mr-1"></a>'+ shareCount+'</span>';
        ...
        <div class="container-fluid ml-1 mt-4 mb-5" id="content" style="min-height: 600px; width: 98%;font-family: 'Montserrat', sans-serif; font-style: normal; font-weight: 300; font-size: 15px;">
            ...
            <div class="row col-12">
                ...
                <div class="col-6 text-justify" id="fbStory">
                    ...
                    <p><br>
                    <strong style="font-weight: 500;font-size: 24px;">Current activities planned for <?php echo $year; ?>:</strong><br>
